# Merge and clean data for NMA
# Outcome: Education completion - high school of equivalent
...
education_es_data_location <- "./network-meta/es-transformation/output/education_es_data.RDS"

education_es_data <- readRDS(education_es_data_location)

# get list of study ids from effect size data
included_studies_education <- education_es_data$study

# filter components for included studies reporting education outcomes
education_component_data <- component_data %>%
  filter(study_reference %in% included_studies_education) 
...
education_treatment_comparison_core_components <- named_education_component_data %>%
  select(
    study,
...
# export data
education_treatment_comparison_core_components %>%
  left_join(
    education_es_data,
    by = "study") %>%
  saveRDS("./network-meta/nma/data/education_analysis_data.RDS")
