...
## MERGE AND CLEAN DATA FOR NMA FOR SAU HETEROGENEITY MODEL
...
education_component_data_location <- "https://docs.google.com/spreadsheets/d/1IlYzDoBuMwCw_AkRILwYB8gYLZWVDGLumYFMmrl7tOk/edit#gid=698569653"

education_component_data <- read_sheet(
  ss = education_component_data_location,
  sheet = "component_combinations")

education_es_data_location <- "./es-transformation/output/education_es_data.RDS"

education_es_data <- readRDS(education_es_data_location)

## RENAME COMPONENT DATA

named_education_component_data <- education_component_data %>%
  filter(study_reference %in% education_es_data$study) %>%
...
subset_education_component_data <- named_education_component_data %>%
...
subset_education_component_data %>%
  saveRDS("./visualisation/input/education_component_breakdown.RDS")

## PREPARE DATA FOR SPECIFICATION ONE: CORE COMPONENTS VERSUS SAU

education_comparison_all_sau <- subset_education_component_data %>%
  select(
    study,                              
    intervention_basic_skills,                    
    intervention_life_skills,                     
...
education_comparison_all_sau_other_breakdown <- subset_education_component_data %>%
  select(
    study,                              
    intervention_basic_skills,                    
    intervention_life_skills,                     
  ...
  ## Remove studies with no active components in treatment group
  filter(
    !intervention == "C-MGMT+P-WRKEXP+COUNSL")

## PREPARE DATA FOR SPECIFICATION THREE: CORE COMPONENTS VERSUS SAU COMPONENT HETEROGENEITY

education_comparison_sau_heterogeneity <- subset_education_component_data %>%
  select(
    study,                              
    intervention_basic_skills,                    
    intervention_life_skills,                     
...
education_comparison_sau_heterogeneity_other_breakdown <- subset_education_component_data %>%
  select(
    study,                              
    intervention_basic_skills,                    
    intervention_life_skills,                     
...
education_comparison_all_sau %>%
  left_join(
    education_es_data,
    by = "study") %>%
  saveRDS("./nma/meta-data/education_analysis_data_specification_one.RDS")

education_comparison_all_sau_other_breakdown %>%
  left_join(
    education_es_data,
    by = "study") %>%
  saveRDS("./nma/meta-data/education_analysis_data_specification_two.RDS")

education_comparison_sau_heterogeneity %>%
  left_join(
    education_es_data,
    by = "study") %>%
  saveRDS("./nma/meta-data/education_analysis_data_specification_three.RDS")

education_comparison_sau_heterogeneity_other_breakdown %>%
  left_join(
    education_es_data,
    by = "study") %>%
  saveRDS("./nma/meta-data/education_analysis_data_specification_four.RDS")
